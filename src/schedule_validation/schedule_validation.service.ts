...
export class ScheduleValidationService {
  ...
  public async checkScheduleConflicts(
    ...
    const conflictingAvailabilities = await this.prisma.availabilitySchedule.findMany({
      where: {
        instructorId,
        AND: [
          {
    ...
    const conflictingUnavailabilities = await this.prisma.instructorUnavailability.findMany({
      where: {
        instructorId,
        AND: [
          {
    ...
    if (conflictingAvailabilities.length > 0) {
      const conflictDates = conflictingAvailabilities.map(
        avail => `${avail.startDateTime.toISOString()} - ${avail.endDateTime.toISOString()}`
      ).join(', ');
    ...
    if (conflictingUnavailabilities.length > 0) {
      const conflictDates = conflictingUnavailabilities.map(
        unavail => `${unavail.startDateTime.toISOString()} - ${unavail.endDateTime.toISOString()}`
      ).join(', ');
    ...
    if (conflictingAppointments.length > 0) {
      const conflictDates = conflictingAppointments.map(
        appt => `${appt.startTime.toISOString()} - ${appt.endTime.toISOString()}`
      ).join(', ');
